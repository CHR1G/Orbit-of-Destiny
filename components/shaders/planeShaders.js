...
export const fragmentShader = /* glsl */ `
  ...
  // focus zoom — in-shader, because the planes are SDFs drawn in one pass
  uniform float uZoom;        // 0..1 master, GSAP-tweened on click
  uniform float uZoomPlane;   // which plane is being focused (-1 = none)
  uniform float uZoomScale;   // focused plane grow factor
  uniform float uZoomShrink;  // non-focused shrink factor
  uniform float uZoomDim;     // non-focused brightness
  uniform float uZoomGoo;     // blend strength under focus
  ...
  void main() {
    ...
    // Under focus the goo relaxes so the enlarged card reads crisp rather
    // than melting into its neighbours. uZoom is 0 while idle, so this is k.
    float k = mix(uK, uK * uZoomGoo, uZoom);
    if (uMouse.z > 0.001) {
    ...
    for (int i = 0; i < MAX_PLANES; i++) {
      ...
      vec4 st = uScale[i];
      // Focus zoom: the targeted plane grows, the rest shrink. uZoom is 0
      // while idle, so this reduces to sc = st.xy and the original look.
      float isFocus = step(0.5, 1.0 - abs(float(i) - uZoomPlane));
      float zf = mix(1.0, mix(uZoomShrink, uZoomScale, isFocus), uZoom);
      vec2 sc = st.xy * zf;
      ...
      if (di < d0) {
        ...
        d0 = di; uv0 = luv; im0 = st.w;
        dm0 = mix(st.z, mix(uZoomDim, 1.0, isFocus), uZoom);
      } else if (di < d1) {
        d1 = di; uv1 = luv; im1 = st.w;
        dm1 = mix(st.z, mix(uZoomDim, 1.0, isFocus), uZoom);
      }
    }
