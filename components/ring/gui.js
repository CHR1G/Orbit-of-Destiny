...
export function mountGui(GUI, { params, state, info, actions }) {
  ...
  stage.add(params, "stageX", 0, 2.5, 0.005).name("move x (radii)");
  ...
  // -- idle cruise + focus zoom --------------------------------------------
  const idle = gui.addFolder("idle cruise");
  idle.add(params, "idleDelay", 0, 10, 0.1).name("start after (s)");
  idle.add(params, "idleRate", 0, 0.3, 0.005).name("drift (rad/s)");

  const zoom = gui.addFolder("focus zoom");
  zoom.add(params, "zoomScale", 1, 5, 0.1).name("grow focused");
  zoom.add(params, "zoomShrink", 0.2, 1, 0.01).name("shrink others");
  zoom.add(params, "zoomDim", 0, 1, 0.01).name("dim others");
  zoom.add(params, "zoomGoo", 0, 1, 0.01).name("goo under focus");
  zoom.add(params, "zoomTime", 0.2, 3, 0.05).name("in/out (s)");
  zoom.add(params, "zoomEase", EASES).name("ease");
