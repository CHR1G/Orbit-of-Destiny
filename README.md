...
> [!NOTE]
> **Deck art provenance (settled 2026-09-14).** All 79 files in `public/tarot/`
> are **AI-generated by the maintainer** — not scans, not tracings, not
> reproductions of the Rider–Waite–Smith deck. The "Rider–Waite–Smith" wording
> in the code describes a layout convention, not an artwork source, so there is
> no upstream copyright claim on the images and nothing blocks redistribution.
> If you swap them for other art, update this note with where the new images
> came from. See [LICENSE](LICENSE) and [docs/tarot-art-spec.md](docs/tarot-art-spec.md).
