...
1. **The `.woff2` files in `@font-face` do not exist.** `globals.css` lists
   `/fonts/PangMenZhengDao-XiXianTi.woff2` and `/fonts/TheNightWatch.woff2` *before*
   the `.ttf` fallbacks, and neither file is in `public/fonts/`. Every visitor makes
   two round trips that 404 before the TTF loads. Dropping real woff2 files in needs
   no code change; subsetting the CJK face would take 1.76 MB to 100–300 KB.
